...
<div class="container py-4">
  <form action="{{ route('income.store') }}" method="post">
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <select name="provider_id" id="provider_id" class="form-select" required >
                ...
                @foreach($provider_id as $provider_id)
                    <option value="{{$provider_id->id}}"> {{$provider_id->Firts_Name}}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="date" class="form-control" name="date" id="date" value="{{$income->date}}" min="2024-01-01" max="2025-12-31">
        </div>
    </div>
    <div class="form-group row">
        <label for="tax" class="col-sm-2 col-form-label">Impuestos</label>
        <div class="col-sm-10">
            <input type="number" class="form-control" name="tax" id="tax" step="0.01" value="{{$income->tax}}" placeholder="Ingresa el valor del Impuesto">
        </div>
    </div>
    <div class="form-group row">
        <label for="total" class="col-sm-2 col-form-label">Total</label>
        <div class="col-sm-10">
            <input type="number" class="form-control" name="total" id="total" step="0.0001" value="{{$income->total}}" placeholder="Ingresa el valor Total">
        </div>
    </div>
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <select class="form-control" name="status" id="status">
                <option value="1" {{ $income->status == 1 ? 'selected' : '' }}>Cancelado</option>
                <option value="0" {{ $income->status == 0 ? 'selected' : '' }}>Pendiente</option>
            </select>
        </div>
    </div>
    <div class="form-group row">
